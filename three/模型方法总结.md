...
```js
    ...
    initModelLabelCss2d() {
      ...
      // 创建多个模型标签
      this.makeLabelCss2d('objName');
      this.makeLabelCss2d('objName');
      
      this.labelRender = new CSS2DRenderer();
      this.labelRender.setSize(window.innerWidth, window.innerHeight);

      document
        .querySelector(".className") //html要先创建对应类名的元素
        .appendChild(this.labelRender.domElement);
    },

    // 模型添加标签处理
    makeLabelCss2d(objName) {
      const obj = this.scene.getObjectByName(objName);
      const label = this.initLabelCss2dProps('div',{
        className: `label`,
      });
    ...
    // 用法
    // this.makeLabelCss2d(obj)

    // 设置元素的属性
    initLabelCss2dProps(type,propsOjb) {
    ...
    // 用法创建div设置对应的属性
    //  const label_text_str = this.initLabelCss2dProps('div',{
